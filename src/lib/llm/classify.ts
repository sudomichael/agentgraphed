...
// Claude Code preamble shapes we don't want feeding the classifier — they
// contain no real intent. Mirror of isClaudeCodePreamble() in the ingester.
...
// Preamble messages (environment context, command stubs, local-command
// caveats) are filtered out — they aren't things the user actually said.
function sampleSessionPrompts(sessionId: string, firstPrompt: string): string[] {
  ...
  const rows = rawRows.filter((r) => !isPreambleContent(r.content));

  if (rows.length === 0) {
    // Fall back to the (already-filtered) firstPrompt only if it looks like
    // real content; otherwise return empty so the classifier doesn't end up
    // titling the session after the preamble.
    return isPreambleContent(firstPrompt) ? [] : [firstPrompt];
  }
