...
- **Multiple tagged log directories per provider.** Settings → Data sources is now an editable list per provider (path + tag) instead of a single directory, so setups with more than one Claude Code home — e.g. the default `~/.claude` plus a work home selected via `CLAUDE_CONFIG_DIR` — get indexed together. Each session is stamped with its source's tag, shown as a color-coded badge on the timeline, session list, and session detail, and filterable via a new source `<select>` on the timeline and sessions pages (only rendered when more than one tag exists). Existing single-directory installs keep working untouched: the legacy `claude_log_dir`/`codex_log_dir` settings and `AGENTGRAPHED_*_DIR` env vars still resolve as the fallback, tagged `default`, and pre-existing sessions are backfilled to `default` by a one-time schema migration. Files reachable through more than one configured source (nested dirs, symlinks) are deduped by real path, first source wins. Editing a tag or path re-tags existing sessions on the next scan.
- **README: troubleshooting section for native `better-sqlite3` install errors** — explains that `ignore-scripts=true` npm setups skip the native build (leading to "Could not locate the bindings file" / `NODE_MODULE_VERSION` errors) and shows the `--ignore-scripts=false` re-install commands plus the build-tool prerequisites.
